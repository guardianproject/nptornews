// Original source at http://code.google.com/p/andro-views/
//
// Copyright 2011 NPR
//
 ...
 * contains a view. A workspace is meant to
...
public class WorkspaceView extends ViewGroup {
  ...
  /**
   * Sets the current screen. If the screen is out of bounds will return
   * show the left- or right-most screen depending on which side it is out of
   * bounds on.
   *
   * @param position An index to the position of the screen to make current
   */
  public void setCurrentScreen(int position) {
    currentScreen = Math.max(0, Math.min(position, getChildCount()));
    scrollTo(currentScreen * getWidth(), 0);
    invalidate();
    if (onScreenChangeListener != null ) {
      onScreenChangeListener.onScreenChanged(position);
    }
  }
  ...
  /**
   * Adds a listener to be notified when the screen changes from a scroll
   * event or programatically.
   *
   * @param l The listener used to respond to screen changes.
   */
  public void setOnScreenChangeListener(OnScreenChangeListener l) {
    this.onScreenChangeListener = l;
  }


  /**
   * A listener interface that gets called when the screen changes in the
   * workspace.
   */
  public interface OnScreenChangeListener {

    /**
     * Called when the Workspace changes the visible screen
     * @param newPosition The position of the new view being shown
     */
    void onScreenChanged(int newPosition);
  }
  ...
  protected void dispatchDraw(Canvas canvas) {
    ...
    if (wallpaper != null) {
      ...
      canvas.drawBitmap(
        wallpaper,
        x,
        (getBottom() - getTop() - wallpaperHeight) / 2,
        paint
      );
    }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (wallpaperLoaded) {
      ...
      wallpaper = centerToFit(wallpaper, width, MeasureSpec.getSize(heightMeasureSpec));
  ...
  public void scrollToScreen(int whichScreen) {
    ...
    invalidate();
    if (changingScreens && onScreenChangeListener != null ) {
      onScreenChangeListener.onScreenChanged(whichScreen);
    }
  }

  /**
   * Return the parcelable instance to be saved
   */
  @Override
   ...
   * @return
   */
  private static Bitmap centerToFit(Bitmap bitmap, int width, int height) {
    ...
    if (bitmapWidth < width || bitmapHeight < height) {
      ...
      Bitmap centered = Bitmap.createBitmap(
          bitmapWidth < width ? width : bitmapWidth,
          bitmapHeight < height ? height : bitmapHeight,
          Bitmap.Config.RGB_565);
      Canvas canvas = new Canvas(centered);
      canvas.drawColor(color);
      canvas.drawBitmap(
          bitmap,
          (width - bitmapWidth) / 2.0f,
          (height - bitmapHeight) / 2.0f,
          null);
      bitmap = centered;
    }
    return bitmap;
