...
public class MainTest extends ActivityInstrumentationTestCase2<Main> {
  private static final String LOG_TAG = MainTest.class.getName();
  private static final long MAX_TEST_RUN = 5000; // seconds

  public MainTest() {
    super("org.npr.android.news", Main.class);
  }
  ...
  public void testShouldStartGPSListenersOnLaunch() {
    ...
    try {
      ...
      Log.d(LOG_TAG, "Main test interrupted waiting for thread to compelete.");
  ...
  private class ListenerInspectionThread extends Thread {
    ...
    public void run() {
      ...
      while ((activity.locationListeners == null ||
          activity.locationListeners.size() == 0)
          && System.currentTimeMillis() < timeStarted + MAX_TEST_RUN) {
        try {
          Thread.sleep(20);
        } catch (InterruptedException e) {
